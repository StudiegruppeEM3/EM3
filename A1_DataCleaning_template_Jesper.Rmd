...
colnames(demo_train)[which(names(demo_train) == "Child.ID")] <- "SUBJ"

demo_train = demo_train %>% rename(SUBJ = q)
...
df = merge(LU_train, token_train, by = c("SUBJ","VISIT"))
df = merge(df, demo_train, by = c("SUBJ","VISIT"))


df = df %>% select(SUBJ, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization,MOT_MLU, CHI_MLU,types_MOT, types_CHI, tokens_MOT, tokens_CHI)
...
```{r}
clinical = df %>% filter(VISIT == 1)

clinical = clinical %>% select(SUBJ, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)



df1 = merge(clinical,df)
...
```{r}
df1$SUBJ = as.factor(df1$SUBJ).
df1$SUBJ = as.numeric(df1$SUBJ)
df1$Gender = ifelse(df1$Gender == 1, "F", "M")
df1$Diagnosis = ifelse(df1$Diagnosis == "A", "ASD", "TD")
